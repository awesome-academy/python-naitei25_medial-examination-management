...
from common.constants import RESET_TOKEN_LENGTH
...
class AuthService:
    ...
    def complete_registration(cls, data):
        ...
        if otp_response.get('resetToken') != 'SUCCESS':
            raise ValidationError(otp_response.get('resetToken', 'OTP validation failed'))
    ...
    def send_reset_password_email(self, email):
        try:
            ...
            send_mail(
                _('Đặt lại mật khẩu'),
                f'Mã đặt lại mật khẩu của bạn là: {reset_token}',
            ...
            raise ValidationError(_("Email không tồn tại trong hệ thống"))
...
class ResetTokenService:
    ...
    def generate_reset_token(user):
        import random
        token = ''.join(random.choices('0123456789', k=RESET_TOKEN_LENGTH))
...
class OtpService:
    ...
    def send_otp_to_email(email):
        ...
        send_mail(
            _('Xác nhận email'),
            f'Mã OTP của bạn là: {otp}. Mã này có hiệu lực trong 10 phút.',
            settings.EMAIL_HOST_USER,
            [email],
            fail_silently=True,
        )
