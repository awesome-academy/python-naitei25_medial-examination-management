...
const DoctorSchedule = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const processedEvents = filteredSchedules
          .map((sch: ScheduleResponse, index) => {
            ...
            const shiftToCalendarType: Record<string, string> = {
              M: "morning",
              A: "afternoon",
              E: "surgery",
              N: "meeting",
            };
            const calendarType = shiftToCalendarType[sch.shift as string] || "morning"; // Default to "morning" if shift is not recognized
        ...
        if (processedEvents.length > 0) {
          const firstEventDate = new Date(processedEvents[0]!.start);
          setCalendarInitialDate(firstEventDate); // Set initial date to the first event's date
  ...
  const calendarToShift: Record<string, string> = {
    morning: "M",
    afternoon: "A",
    surgery: "E",
    meeting: "N",
  ...
  const handleAddEvent = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        shift: calendarToShift[newEvent.calendar],
        room:
