...
const PatientDetail: React.FC = () => {
  ...
  useEffect(() => {
    if (patientDetail && (!userHasModified || !formInitialized)) {
      const formValues: any = {
        name: `${patientDetail.patientInfo?.first_name || ""} ${patientDetail.patientInfo?.last_name || ""}`.trim() || t("labels.unknownPatient"),
      ...
      if (prescription && prescription.appointment === appointmentId) {
        Object.assign(formValues, {
          ...
          systolicBloodPressure: prescription?.systolic_blood_pressure || undefined,
          diastolicBloodPressure: prescription?.diastolic_blood_pressure || undefined,
  ...
  }, [patientDetail, prescription, appointmentId, form, t, userHasModified, formInitialized]);
  ...
  <div className="space-y-4">
    {prescriptionsForCurrentAppointment.map((prescriptionItem) => (
      <div
        ...
        <div className="flex items-center mb-2">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium">
              {t("labels.prescription")} #{prescriptionItem.id}
            </p>
            ...
            <p className="text-xs text-gray-500">
              ...
              {prescriptionItem.prescription_details?.length || 0}
            </p>
            <p className="text-xs text-gray-500">
              {t("labels.diagnosis")}:{" "}
              {prescriptionItem.diagnosis || t("labels.noDiagnosis")}
            </p>
          </div>
          <Button
            type="text"
        ...
        {prescriptionItem.is_follow_up && (
          <p className="text-xs text-blue-500">
            ...
            <div className="w-full lg:w-3/4">
              <Form form={form} layout="vertical" onFieldsChange={handleFieldChange}>
                <Row gutter={24}>
                  <Col span={12}>
                  ...
                  <TabPane tab={t("tabs.prescriptions")} key="3">
                    <div className="mb-6">
                      <div className="flex justify-between items-center mb-4">
                        ...
                        {!isCompleted && (!prescription || prescriptionsForCurrentAppointment.length === 0) && (
                        ...
                        <div className="space-y-4">
                          {prescriptionsForCurrentAppointment.map((prescriptionItem) => (
                            ...
                            >
                              <div className="flex items-center mb-2">
                                ...
                                {!isCompleted && (
                                  <Button
                                    type="primary"
                                    onClick={() => {
                                      setSelectedPrescription(prescriptionItem);
                              ...
                              {prescriptionItem.is_follow_up && (
                                <p className="text-xs text-blue-500">
                                  {t("labels.followUpAppointment")}: {prescriptionItem.follow_up_date ? formatDate(prescriptionItem.follow_up_date) : t("labels.yes")}
                                </p>
                              )}
                            </div>
                          ))}
